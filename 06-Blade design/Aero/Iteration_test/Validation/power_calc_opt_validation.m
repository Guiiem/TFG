function Book =power_calc_opt_validation(Book, data,chord,twist, iprint, m)
...
Re_array=data.re_array(3); % Re array
num_re=length(Re_array); % length of Re array
Re_log=data.re_log(3); % log(Re) array
...
rad=data.rad*r_tip;  % Radius of blade elements
...
for i = 1: nbes
      ...
      for j = 1:100 % Limit the  number of iterations per BE to 20
...
Book.Cp(m) = torque*lambda;
if Book.Cp(m) > 0.593; Book.Cp(m)=0.005;end

Book.Cq(m) = torque;
Book.Ct(m) = thrust;
Book.lambda(m) = lambda;

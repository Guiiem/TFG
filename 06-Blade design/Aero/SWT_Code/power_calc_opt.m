function cp=power_calc_opt(data,chord,twist, iprint)
...
a_c = 1/3.0; % Paramete rfor high thrust correction
...
for i = 1: nbes
      ...
      for j = 1:20 % Limit the  number of iterations per BE to 20
              ...
              cosphi = cos(phi);
              sinphi = sin(phi);
              bigF = 2*acos(exp(-smallf(i)/sinphi))/pi; % Equation **
%              bigF = 1.0;
              ...
              C_a = Cl*cosphi + Cd*sinphi;
              Y1 = 4*bigF*sinphi^2/sigma(i)/C_a;   
 ...
 %             if (Y1 >= 1.0 ) % In the low thrust region
              ...
              diffa = abs(a - newa); 
              a = real(newa);
              C_adash = Cl*sinphi - Cd*cosphi; % Resultant in circumferential direction
